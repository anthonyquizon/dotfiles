...
class Source(Base):
    ...
    def __init__(self, vim):
        ...
        self.name = 'directory_rec/git'
        self.kind = 'directory_git'
        self.vars = {
            'command': [],
    ...
    def __async_gather_candidates(self, context, timeout):
        ...
        if isabs(outs[0]):
            candidates = [{
                'word': sub(r'.git/', '', relpath(x, start=context['__directory'])),
                'abbr': sub(r'.git/', '', relpath(x, start=context['__directory']) + '/'),
                'action__path': sub(r'.git/', '', x)
